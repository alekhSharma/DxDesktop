...
const OrgInfo = require(path.join(__dirname, 'scratchOrgInfoApi'));
...
const _buildOrganizationSettingsDetailToolingCreateSobject = function (orgApi, prefs) {
    ...
    return orgApi.force.toolingQuery(orgApi, query)
        ...
        if (!result.records || !result.records[0]) {
            const err = new Error(messages.getMessage('ProblemQueryingOrganizationSettingsDetail'));
...
const _buildNoOrgError = org => {
    ...
    if (org.type === SfdxConfig.OrgDefaults.USERNAME) {
        noConfigError.action = messages.getMessage('defaultOrgNotFoundAction');
    }
    else if (org.type === SfdxConfig.OrgDefaults.DEVHUB) {
        noConfigError.action = messages.getMessage('defaultOrgNotFoundDevHubAction');
    }
    ...
    return metaConfigs.reduce((accum, currentValue) => {
        ...
        if (_.isError(currentValue[2])) {
            ...
            accum.mergedScratchOrgInfosAcrossLocalDevHubs =
                new Map([...accum.mergedScratchOrgInfosAcrossLocalDevHubs, ...currentValue[2]]);
        }
        else {
 ...
 * @private
 */
const _groupOrgs = function (configsAcrossDevHubs, orgClass, excludeProperties) {
    const self = this;
    ...
    if (_.isNil(configsAcrossDevHubs.orgs)) {
        return Promise.resolve(_accum);
    }
    return aggregator.initialize()
        .then(() => Promise.all([
        orgClass.create(undefined, orgClass.Defaults.DEVHUB).then(org => org.name).catch(() => null),
        orgClass.create(undefined, orgClass.Defaults.USERNAME).then(org => org.name).catch(() => null)
    ]))
        .then((defaultOrgs) => configsAcrossDevHubs.orgs.reduce((accum, _currentValue) => {
...
const _updateOrgIndicators = function (groupedOrgs, _orgUpdate) {
    return Promise.map(groupedOrgs.scratchOrgs, (meta) => _orgUpdate(meta[1], { devHubUsername: meta[1].devHubUsername }), { concurrency: 1 })
        .catch((err) => {
...
class Org {
    ...
    retrieveMaxApiVersion() {
        ...
        return this.force.getApiVersions(this)
            .then((versions) => _.maxBy(versions, (_ver) => _ver.version));
    }
    /**
    ...
    resolveDefaultName() {
        ...
        return this.resolvedAggregator().then(sfdxConfig => {
            const name = sfdxConfig.get(this.type);
    ...
    resolvedAggregator() {
        if (this.aggregator) {
            return Promise.resolve(this.aggregator);
        }
        this.aggregator = new SfdxConfigAggregator();
        return this.aggregator.initialize().then(() => this.aggregator);
    }
    /**
     ...
     */
    isWorkspaceOrg() {
        return this.type === SfdxConfig.OrgDefaults.USERNAME && this.getName() === this.config.getAppConfigIfInWorkspace()[this.type];
    }
    ...
    getConfig() {
        ...
        return this.resolveDefaultName()
            ...
            if (accessTokenMatch) {
                ...
                return Promise.resolve(srcDevUtil.getGlobalConfig(`${username}.json`))
                    .then((config) => configValidator.getCleanObject(config, orgConfigAttributes, false))
                    .then((config) => {
                    if (_.isNil(config.clientId)) {
                        config.clientId = defaultConnectedAppInfo.clientId;
                        config.clientSecret = defaultConnectedAppInfo.clientSecret;
                    }
                    return config;
                })
                    .catch((error) => {
     ...
     * @returns {Promise.<Object>} Not the access tokens will be encrypted. Call get config to get decrypted access tokens.
     */
    saveConfig(configObject, saveAsDefault) {
        ...
        return configValidator.getCleanObject(configObject, orgConfigAttributes, true)
            .then((dataToSave) => {
            if (dataToSave.clientId === defaultConnectedAppInfo.clientId) {
                dataToSave.clientId = undefined;
                dataToSave.clientSecret = undefined;
            }
        ...
        if (!_.isNil(prefsMap)) {
            return Promise.each(prefsMap.entries(), (pref) => {
                ...
                return retVal.then((toolingCreateSobjectArr) => {
                    ...
                    const error = _.first(results.filter(result => !result.success));
                    if (error) {
                        return Promise.reject(almError('ProblemSettingOrgPrefs', [(error && error.error) || error]));
                    }
                    return results;
                }).catch((err) => Promise.reject(err));
            });
    ...
    static hasAuthentications() {
        return Org.readAllUserFilenames().then(users => !_.isEmpty(users)).catch((err) => {
            // ENOENT
    ...
    static readAllUserFilenames() {
        return fs.readdirAsync(srcDevUtil.getGlobalHiddenFolder())
            .then((files) => {
            ...
            if (_.isEmpty(sfdxFiles)) {
                throw almError({ keyName: 'noOrgsFound', bundle: 'scratchOrgApi' }, null, { keyName: 'noOrgsFoundAction', bundle: 'scratchOrgApi' }, null);
            }
            else {
    ...
    static readMapOfLocallyValidatedMetaConfig(userFilenames, concurrency = 3) {
        if (concurrency < 1) {
            return Promise.reject(new Error('concurrency setting must be greater than zero.'));
        }
        const orgInfo = new OrgInfo();
        if (_.isNil(userFilenames) || _.isEmpty(userFilenames)) {
            return Promise.reject(new Error('file names not specified.'));
        }
        const fileDoesntExistFilter = Promise.all(_.map(userFilenames, (fileName) => {
            const filePath = path.join(srcDevUtil.getGlobalHiddenFolder(), fileName);
            return Promise.all([
                fs.statAsync(filePath).then(stat => stat).catch(() => null),
                srcDevUtil.readJSON(filePath).then(content => content).catch((err) => {
            ...
            .filter((statsAndContent) => !_.isNil(statsAndContent[0]) && !_.isNil(statsAndContent[1]) &&
            _.isNil(statsAndContent[1].scratchAdminUsername));
        return fileDoesntExistFilter.then((fileContentsAndMeta) => {
            ...
            return Promise.map(fileContentsAndMeta, (fileContentAndMeta) => {
                /**
                ...
                const promiseDevHubMetadata = (username) => {
                    const org = new Org(this.force);
                    org.setName(username);
                    return org.getConfig().then((configData) => {
                        ...
                        if (_.isNil(configData.devHubUsername)) {
                            return org.refreshAuth()
                                ...
                                .catch((err) => {
                                org.logger.trace(`error retrieving ScratchOrgInfo object for org: ${configData.username}. this is expected for non-devhubs`);
                                org.logger.trace(err);
                                return err;
                            });
                        }
                ...
                return Promise.all([fileContentAndMeta[0], fileContentAndMeta[1],
    ...
    static readLocallyValidatedMetaConfigsGroupedByOrgType(userFilenames, concurrency = 3, excludeProperties) {
        return Org.readMapOfLocallyValidatedMetaConfig(userFilenames, concurrency)
            .then((configsAcrossDevHubs) => Alias.list()
            .then((aliases) => {
            ...
            .then(configsAcrossDevHubsWithAlias => _groupOrgs.call(new Config(), configsAcrossDevHubsWithAlias, Org, excludeProperties))
            .then((groupedOrgs) => _updateOrgIndicators.call({ logger: logger.child('readLocallyValidatedMetaConfigGroupdByOrgType') }, groupedOrgs, (meta, attributesToMerge) => {
            const org = new Org();
     ...
     *
     */
    static create(username, defaultType) {
        ...
        if (_.isString(username) && !_.isEmpty(username)) {
            ...
            promise = Alias.get(username).then(actualUsername => {
        ...
        return promise.then(() => org.getConfig()).then(() => org);
    }
}
/**
 ...
 */
Org.Defaults = SfdxConfig.OrgDefaults;
module.exports = Org;
